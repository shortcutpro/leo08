...
(function(){
  try{
    ...
    if(!d.querySelector('meta[name="viewport"]')){
      ...
      (d.head||d.getElementsByTagName('head')[0]||d.documentElement).appendChild(mv);
    }
  }catch(e){}
})();
...
var SITE_NAME     = 'KUATBET';
var SITE_LOGO     = 'https://kuatbet.com/stream/id?attachId=5131576';
var MBAPPE_GIF    = 'https://photoku.io/images/2026/05/31/giffmbappee-finall.gif';
var MARQUEE_TEXT  = '👑 PREDIKSI BOLA TERUPDATE ! Tunggu apa lagi? Daftar di KUATBET dan nikmati pengalaman taruhan terbaik dengan peluang besar! 🔥';
var COLOR_MAIN    = '#FFD700';
var COLOR_TEXT    = '#fffbd0';
var BG_IMAGE      = 'https://i.postimg.cc/zXLLHWKp/BG-WEB-KUATBET.png';
var BG_OVERLAY    = 'rgba(0,0,0,0), rgba(0,0,0,0)';
var AUTO_REFRESH  = 5 * 60 * 1000; // 5 menit
var CACHE_KEY     = 'kuatbet_pred_cache_v1';
...
/* ═══════════════════════════════════════════════
   ELEMEN ROOT
═══════════════════════════════════════════════ */
var root = document.getElementById('kuatbet-root');

// Inject progress bar CSS
(function(){
  var s=document.createElement('style');
  s.textContent=
    '#kuatbet-root .ibc-progress-wrap{width:80%;max-width:320px;margin-top:2px;}'
    +'#kuatbet-root .ibc-progress-track{width:100%;height:10px;background:rgba(255,255,255,0.06);border-radius:8px;overflow:hidden;border:1px solid #FFD70026;position:relative;}'
    +'#kuatbet-root .ibc-progress-fill{height:100%;width:0%;background:linear-gradient(90deg,#CCB000,#FFD700,#FFE840);border-radius:8px;transition:width .4s ease;box-shadow:0 0 12px #FFD70066,inset 0 1px 0 rgba(255,255,255,0.25);}'
    +'#kuatbet-root .ibc-progress-pct{color:#FFD700;font-size:12px;font-weight:700;letter-spacing:1px;text-align:center;margin-top:6px;text-shadow:0 0 8px #FFD70060;}';
...
function showLoading(msg, pct){
  ...
  root.innerHTML =
    ...
    '<img src="https://kuatbet.com/stream/id?attachId=5131576" alt="KUATBET" style="width:110px;max-height:72px;object-fit:contain;filter:drop-shadow(0 0 16px #FFD700);margin-bottom:8px;"/>'+
    '<div class="ibc-spinner"></div>'+
    '<div class="ibc-loading-text">⚽ '+(msg||'Memuat Prediksi KUATBET…')+'</div>'+
    (hasBar?
...
async function fetchHTML(url){
  ...
  for(var i=0;i<totalProxies;i++){
    ...
    try{
      ...
      console.log('[KUATBET] Mencoba '+px.name+'…');
      ...
      console.log('[KUATBET] OK via '+px.name);
      updateProgress(60);
      return html;
    }catch(e){
      fetchDone = true;
      console.warn('[KUATBET] '+px.name+' gagal:',e.message);
    }
  }
...
function getLogoUrl(teamName){
  ...
  // 1. Fuzzy 8-step — SAMA PERSIS dengan generator (kuatbet.html dkk)
...
function buildOutputHTML(leagues){
  ...
  return '<!-- Generated by KUATBET Auto Embed -->\n'
+'<style>\n'
+'*,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}\n'
+':root{--g:'+bg+';--gl:'+ltr+';--g2:'+drk+';--gd:'+bg+'66;--gs:'+bg+'22;--cr:'+txt+';--bg:#000000;--bg2:#0d0d0d;}\n'
+'body{background:linear-gradient(135deg,'+BG_OVERLAY+'),url("'+BG_IMAGE+'") center/cover fixed no-repeat transparent;font-family:\'Poppins\',sans-serif;color:var(--cr);min-height:100vh;overflow-x:hidden;}\n'
+'@media(max-width:768px){body{background:linear-gradient(135deg,'+BG_OVERLAY+'),url("https://i.postimg.cc/4NFSBCKn/BG-MOBILE-KUATBET.png") center/cover fixed no-repeat transparent;}}\n'
+'#kuatbet-root{background:linear-gradient(135deg,'+BG_OVERLAY+'),url("'+BG_IMAGE+'") center/cover fixed no-repeat transparent;padding:1px 0;border-radius:0;min-height:100px;}\n'
...
async function loadAndRender(){
  ...
  if(cache && cache.html){
    ...
    try {
      ...
      writeCache(freshBg);
      injectHTML(freshBg);
    } catch(e){ console.warn('[KUATBET] refresh latar gagal:', e.message); }
    return;
  }

  // Belum ada cache (kunjungan pertama) — baru tampilkan loading
  try {
    showLoading('Memuat Prediksi KUATBET…', 0);
    var output = await fetchFreshOutput(true);
    writeCache(output);
    injectHTML(output);
  } catch(e) {
    console.error('[KUATBET Embed]',e);
...
async function backgroundRefresh(){
  try {
    ...
    writeCache(fresh);
    injectHTML(fresh);
  } catch(e){ console.warn('[KUATBET] auto-refresh gagal:', e.message); }
}
