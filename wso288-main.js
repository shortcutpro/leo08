...
var COLOR_TEXT    = '#d8ffd0';
var BG_IMAGE      = 'https://i.postimg.cc/yxGkrY4t/BG-WEB-WSO288.png';
...
/* ═══════════════════════════════════════════════
   ELEMEN ROOT
═══════════════════════════════════════════════ */
var root = document.getElementById('wso288-root');

// Inject progress bar CSS
(function(){
  var s=document.createElement('style');
  s.textContent=
    '#wso288-root .ibc-progress-wrap{width:80%;max-width:320px;margin-top:2px;}'
    +'#wso288-root .ibc-progress-track{width:100%;height:10px;background:rgba(255,255,255,0.06);border-radius:8px;overflow:hidden;border:1px solid #5DC82026;position:relative;}'
    +'#wso288-root .ibc-progress-fill{height:100%;width:0%;background:linear-gradient(90deg,#3DA000,#5DC820,#7DE840);border-radius:8px;transition:width .4s ease;box-shadow:0 0 12px #5DC82066,inset 0 1px 0 rgba(255,255,255,0.25);}'
    +'#wso288-root .ibc-progress-pct{color:#5DC820;font-size:12px;font-weight:700;letter-spacing:1px;text-align:center;margin-top:6px;text-shadow:0 0 8px #5DC82060;}';
...
async function fetchHTML(url){
  ...
  for(var i=0;i<totalProxies;i++){
    ...
    try{
      ...
      console.log('[WSO288] Mencoba '+px.name+'…');
      ...
      console.log('[WSO288] OK via '+px.name);
      updateProgress(60);
      return html;
    }catch(e){
      fetchDone = true;
      console.warn('[WSO288] '+px.name+' gagal:',e.message);
    }
  }
...
function getLogoUrl(teamName){
  ...
  // 1. Fuzzy 8-step — SAMA PERSIS dengan generator (wso288.html dkk)
...
function buildOutputHTML(leagues){
  ...
  return '<!-- Generated by WSO288 Auto Embed -->\n'
+'<style>\n'
...
+'@media(max-width:768px){body{background:linear-gradient(135deg,'+BG_OVERLAY+'),url("https://i.postimg.cc/TYH4C8yY/BG-MOBILE-WSO288.png") center/cover fixed no-repeat transparent;}}\n'
+'#wso288-root{background:linear-gradient(135deg,'+BG_OVERLAY+'),url("'+BG_IMAGE+'") center/cover fixed no-repeat transparent;padding:1px 0;border-radius:0;min-height:100px;}\n'
