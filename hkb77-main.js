...
var BG_IMAGE      = 'https://i.postimg.cc/Yq44LgYx/BG-WEB-HKB77.png';
var BG_OVERLAY    = 'rgba(0,0,0,0), rgba(0,0,0,0)';
var AUTO_REFRESH  = 5 * 60 * 1000; // 5 menit
var CACHE_KEY     = 'hkb77_pred_cache_v1';
...
/* ═══════════════════════════════════════════════
   ELEMEN ROOT
═══════════════════════════════════════════════ */
var root = document.getElementById('hkb77-root');

// Inject progress bar CSS
(function(){
  var s=document.createElement('style');
  s.textContent=
    '#hkb77-root .ibc-progress-wrap{width:80%;max-width:320px;margin-top:2px;}'
    +'#hkb77-root .ibc-progress-track{width:100%;height:10px;background:rgba(255,255,255,0.06);border-radius:8px;overflow:hidden;border:1px solid #FFA50026;position:relative;}'
    +'#hkb77-root .ibc-progress-fill{height:100%;width:0%;background:linear-gradient(90deg,#CC8000,#FFA500,#FFC040);border-radius:8px;transition:width .4s ease;box-shadow:0 0 12px #FFA50066,inset 0 1px 0 rgba(255,255,255,0.25);}'
    +'#hkb77-root .ibc-progress-pct{color:#FFA500;font-size:12px;font-weight:700;letter-spacing:1px;text-align:center;margin-top:6px;text-shadow:0 0 8px #FFA50060;}';
...
function showLoading(msg, pct){
  ...
  root.innerHTML =
    ...
    '<img src="https://api2-hk7.imgzm.com/images/rnfdphnj0/logo_9349a711-3efb-4e15-8115-cb6c54969389_1779355053423.png" alt="HKB77" style="width:110px;max-height:72px;object-fit:contain;filter:drop-shadow(0 0 16px #FFA500);margin-bottom:8px;"/>'+
    '<div class="ibc-spinner"></div>'+
    '<div class="ibc-loading-text">⚽ '+(msg||'Memuat Prediksi HKB77…')+'</div>'+
    (hasBar?
...
async function fetchHTML(url){
  ...
  for(var i=0;i<totalProxies;i++){
    ...
    try{
      ...
      console.log('[HKB77] Mencoba '+px.name+'…');
      ...
      console.log('[HKB77] OK via '+px.name);
      updateProgress(60);
      return html;
    }catch(e){
      fetchDone = true;
      console.warn('[HKB77] '+px.name+' gagal:',e.message);
    }
  }
...
function buildOutputHTML(leagues){
  ...
  return '<!-- Generated by HKB77 Auto Embed -->\n'
+'<style>\n'
...
+'@media(max-width:768px){body{background:linear-gradient(135deg,'+BG_OVERLAY+'),url("https://i.postimg.cc/44PMHHs5/BG-MOBILE-HKB77.png") center/cover fixed no-repeat transparent;}}\n'
+'#hkb77-root{background:linear-gradient(135deg,'+BG_OVERLAY+'),url("'+BG_IMAGE+'") center/cover fixed no-repeat transparent;padding:1px 0;border-radius:0;min-height:100px;}\n'
...
async function loadAndRender(){
  ...
  if(cache && cache.html){
    ...
    try {
      ...
      writeCache(freshBg);
      injectHTML(freshBg);
    } catch(e){ console.warn('[HKB77] refresh latar gagal:', e.message); }
    return;
  }

  // Belum ada cache (kunjungan pertama) — baru tampilkan loading
  try {
    showLoading('Memuat Prediksi HKB77…', 0);
    var output = await fetchFreshOutput(true);
    writeCache(output);
    injectHTML(output);
  } catch(e) {
    console.error('[HKB77 Embed]',e);
...
async function backgroundRefresh(){
  try {
    ...
    writeCache(fresh);
    injectHTML(fresh);
  } catch(e){ console.warn('[HKB77] auto-refresh gagal:', e.message); }
}
