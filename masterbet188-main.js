...
var COLOR_TEXT    = '#fff8d0';
var BG_IMAGE      = 'https://i.postimg.cc/yYJJ39cX/BG-WEB-Masterbet188.png';
var BG_OVERLAY    = 'rgba(0,0,0,0), rgba(0,0,0,0)';
var AUTO_REFRESH  = 5 * 60 * 1000; // 5 menit
var CACHE_KEY     = 'masterbet188_pred_cache_v1';
...
(function(){
  ...
  s.textContent=
    '#masterbet188-root .ibc-progress-wrap{width:80%;max-width:320px;margin-top:2px;}'
    +'#masterbet188-root .ibc-progress-track{width:100%;height:10px;background:rgba(255,255,255,0.06);border-radius:8px;overflow:hidden;border:1px solid #C9A84C26;position:relative;}'
    +'#masterbet188-root .ibc-progress-fill{height:100%;width:0%;background:linear-gradient(90deg,#A0803A,#C9A84C,#E0C070);border-radius:8px;transition:width .4s ease;box-shadow:0 0 12px #C9A84C66,inset 0 1px 0 rgba(255,255,255,0.25);}'
    +'#masterbet188-root .ibc-progress-pct{color:#C9A84C;font-size:12px;font-weight:700;letter-spacing:1px;text-align:center;margin-top:6px;text-shadow:0 0 8px #C9A84C60;}';
...
async function fetchHTML(url){
  ...
  for(var i=0;i<totalProxies;i++){
    ...
    try{
      ...
      console.log('[MASTERBET188] Mencoba '+px.name+'…');
      ...
      console.log('[MASTERBET188] OK via '+px.name);
      updateProgress(60);
      return html;
    }catch(e){
      fetchDone = true;
      console.warn('[MASTERBET188] '+px.name+' gagal:',e.message);
    }
  }
...
function getLogoUrl(teamName){
  ...
  // 1. Fuzzy 8-step — SAMA PERSIS dengan generator (masterbet188.html dkk)
...
function buildOutputHTML(leagues){
  ...
  return '<!-- Generated by MASTERBET188 Auto Embed -->\n'
+'<style>\n'
...
+'@media(max-width:768px){body{background:linear-gradient(135deg,'+BG_OVERLAY+'),url("https://i.postimg.cc/vHSN3R1T/BG-MOBILE-Masterbet188.png") center/cover fixed no-repeat transparent;}}\n'
+'#masterbet188-root{background:linear-gradient(135deg,'+BG_OVERLAY+'),url("'+BG_IMAGE+'") center/cover fixed no-repeat transparent;padding:1px 0;border-radius:0;min-height:100px;}\n'
...
async function loadAndRender(){
  ...
  if(cache && cache.html){
    ...
    try {
      ...
      writeCache(freshBg);
      injectHTML(freshBg);
    } catch(e){ console.warn('[MASTERBET188] refresh latar gagal:', e.message); }
    return;
  }

  // Belum ada cache (kunjungan pertama) — baru tampilkan loading
  try {
    showLoading('Memuat Prediksi MASTERBET188…', 0);
    var output = await fetchFreshOutput(true);
    writeCache(output);
    injectHTML(output);
  } catch(e) {
    console.error('[MASTERBET188 Embed]',e);
...
async function backgroundRefresh(){
  try {
    ...
    writeCache(fresh);
    injectHTML(fresh);
  } catch(e){ console.warn('[MASTERBET188] auto-refresh gagal:', e.message); }
}
